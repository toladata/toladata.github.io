module Tola
  ...
  def all_features_for(product)
    get_child(product, 'features').children.sort { |a, b| a.data.position <=> b.data.position }
  end

  def all_pages_for(product)
    get_child(product, 'pages').children.sort { |a, b| a.data.position <=> b.data.position }
  end

  def all_products
    get_page('products').children
  end
